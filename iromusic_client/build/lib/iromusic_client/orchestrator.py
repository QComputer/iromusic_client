...
class RunStats:
    ...
    api_stats: RequestStats = None
    
    def __post_init__(self):
        if self.api_stats is None:
            self.api_stats = RequestStats()
...
class Orchestrator:
    ...
    def run_endpoint(
        ...
        try:
            ...
            if save and items:
                ...
                # Use processed data if available, otherwise use original items
                data_to_save = result.get('processed_data')
    ...
    def run_all_endpoints(
        ...
        try:
            ...
            self.stats.api_stats.failed_requests = self.client.stats.failed_requests
